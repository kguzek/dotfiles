...
description: |
  ...
  - Managing channels (create + invite users)
...
## Workspace profile (local/sensitive)

If present, consult `references/local-kenley-workspace.md` before choosing channels. Keep this file local and uncommitted (it is gitignored in this repo).

Refresh it with:

```bash
scripts/refresh-local-kenley-workspace-context --workspace kenley-ai
```

This script uses `codex exec` as a subagent to synthesize the markdown from read-only channel/activity metadata.

When working in short-lived channels (for example `*-short-lived`), assume customer data may appear and retention may be low. Prefer concise summaries and avoid copying raw sensitive details unless explicitly requested.
...
`channel list` is not available in released `agent-slack` `v0.4.3` (the current latest release); use `message list`, `search`, and `user list` for read-oriented workflows.
